...
cc.Class({
    ...
    onEnable(){
        this._super()
        this.m_config = Gm.config.getAccumulativePayConfig(AtyFunc.FIRS_PAY_TYPE)
        this.initView()
        this.updateView()
    },
    ...
    initView(){
        ...
        for(let key in this.m_btnList){
            Gm.red.add(this.m_btnList[key].node,"firstPay","firstPaybtn_"+key)
        ...
        Gm.load.loadSpriteFrame("img/FCharge/"+this.m_config[0].titleImage,function(sp,icon){
            icon.spriteFrame = sp
        },this.m_titleImg)

        var canReceiveId = Gm.firstPayData.getCanReceiveId()
    ...
    updateView(){
        ...
        var diffPrice = conf.cost - Gm.userInfo.getFirstPayTotal()
        var satus = Gm.firstPayData.isReceiveFirstActivityReward(conf.id) 
        if(diffPrice > 0){
        ...
        Gm.load.loadSpriteFrame("img/FCharge/"+conf.LiHui,function(sp,icon){
            icon.spriteFrame = sp
        },this.m_bgSprite)
    ...
    updateBtn(id){
        var status = Gm.firstPayData.isReceiveFirstActivityReward(id)
        if(status == 1){
    ...
    onPlayBattleClick(){
        Gm.ui.create("BattleLoadView",Gm.config.getAccuPayBattleById(this.m_currentSelectIndex))
    },

    onBtn(){
        var conf = this.m_config[this.m_currentSelectIndex]
        var status = Gm.firstPayData.isReceiveFirstActivityReward(conf.id)
