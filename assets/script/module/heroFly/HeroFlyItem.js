cc.Class({
    ...
    onEnable(){
        if (this.isEnter){
            this.isEnter = false
            this.node.scale = 0.7
            this.node.runAction(cc.scaleTo(0.1,1).easing(cc.easeBackOut()))
        }else{
            this.node.scale = 1
        }
    ...
    isMaxQuality(){
        ...
        return this.data.qualityId == maxQuality || this.data.quality == Gm.config.getConst("hero_synthetise_quality_max")
    },
